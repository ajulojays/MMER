...
pA <- ggplot(analysis, aes(study, ecological_resistance)) +
  geom_boxplot(width = 0.48, outlier.shape = NA, linewidth = 0.5) +
...
# Panel B: frozen PC2 association, with study-specific descriptive lines.
pB <- ggplot(analysis, aes(z_PC2, ecological_resistance, shape = study, linetype = study)) +
  ...
  annotate(
    ...
    hjust = -0.03, vjust = 1.15, size = 3.1
  ) +
  labs(
    x = "Baseline ecological state PC2 (SD)",
    y = "Ecological resistance",
    title = "B  Baseline ecological state predicts resistance",
    shape = NULL,
    linetype = NULL
  ) +
  theme_classic(base_size = 10.5) +
...
pC <- ggplot(tertile_summary, aes(PC2_tertile, mean, group = study, shape = study, linetype = study)) +
  ...
  labs(
    ...
    title = "C  Directional resistance gradient in both cohorts",
    shape = NULL,
    linetype = NULL
  ) +
  theme_classic(base_size = 10.5) +
...
# Panel D: compositional interpretation of the frozen PC2 axis.
heat <- family_summary %>%
  filter(family %in% selected$family) %>%
  mutate(
    family = factor(family, levels = family_order),
    PC2_tertile = factor(PC2_tertile,
                         levels = c("Low_PC2", "Mid_PC2", "High_PC2"),
                         labels = c("Low", "Intermediate", "High")),
    group = paste(study, PC2_tertile, sep = "\n"),
...
pD <- ggplot(heat, aes(group, family, fill = mean_RA_percent)) +
  ...
  labs(
    ...
    y = NULL,
    fill = "Mean baseline\nrelative abundance (%)",
    title = "D  Families defining the PC2 ecological-state gradient"
  ) +
...
draw_four_panel <- function(filename, device = c("pdf", "svg", "png")) {
  ...
  if (device == "pdf") {
    pdf(filename, width = 11.2, height = 9.0, useDingbats = FALSE)
  } else if (device == "svg") {
    svg(filename, width = 11.2, height = 9.0)
  } else {
    png(filename, width = 11.2, height = 9.0, units = "in", res = 600)
  }
